    ...
    <View>
      ...
      <View style={styles.card}>
        ...
        <View style={[styles.row, { marginTop: 8, flexWrap: 'wrap' }]}>
          ...
          <Text onPress={resetToSeed} style={{ color: colors.accent, fontWeight: '600' }}>
            Nulstil demo-data
          </Text>
        </View>
      </View>
      ...
      <View style={[styles.card, { marginTop: 12 }]}>
        ...
        <View style={[styles.row, { marginTop: 8, flexWrap: 'wrap' }]}>
          {rooms.map((r) => (
            <Chip key={`room-${r}`} label={r} selected={roomFilter === r} onPress={() => setRoomFilter(r)} />
          ))}
        </View>
        ...
        <View style={[styles.row, { marginTop: 8, flexWrap: 'wrap' }]}>
          {persons.map((p) => (
            <Chip key={`person-${p}`} label={p} selected={personFilter === p} onPress={() => setPersonFilter(p)} />
          ))}
        </View>
      </View>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.card}>
            ...
            <Text style={styles.paragraph}>Booket af: {item.by || 'ukendt'}</Text>
